<template>
  ...
  >
    ...
    <v-row>
      <v-col cols="12">
       <p align=justify>{{ messages.message }}</p>
      </v-col>
    </v-row>
    ...
    >
      <v-row v-if="messages.errors">
        <v-col cols="12" v-for="error in messages.errors" :key="error.field">
          <v-btn x-small text v-scroll-to="`#${error.referenceId}`">
            <v-icon left>
              icon-alert-2
            </v-icon>
            <span :title="`${error.field} ${error.type}` ">{{ error.field }}({{ error.type }})</span>
          </v-btn>
        </v-col>
      </v-row>
    </div>
    <v-row v-if="messages.actions">
      <v-col cols="4" offset="2" class="text-center">
        <v-btn rounded block light class="white">NÃO</v-btn>
      </v-col>
      <v-col cols="4">
        <v-btn rounded block class="warning">SIM</v-btn>
      </v-col>
    </v-row>
    <slot></slot>
  </v-dialog>
